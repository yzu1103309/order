...
function buildTakeSelectQuery(): string
{
    return "SELECT * FROM `Orders`";
}

function normalizeTakeNum($type, string $num): string
{
    if ((int)$type === 2) {
        return $num . "號";
    }
...
function buildTakePages(array $row, array $items, int $itemCount, array $typeName): array
{
    $dish = explode(',', $row[2]);
    $entries = [];
    ...
    $displayNum = normalizeTakeNum($row[0], $row[1]);

    foreach ($pages as $index => $pageEntries) {
        $hasMore = $index < $pageCount - 1;
        $htmlPages[] = buildTakePageHtml(
            (int)$row[0],
            $typeName[(int)$row[0]],
            $displayNum,
            $pageEntries,
...
function buildTakePageHtml(
    ...
    if ($hasMore) {
        ...
        $html .= '
                ...
                <input id="send" name="send" type="button" value="製作完成" class="btn" onclick="complete(' . $auto . ')" style="margin-left: 32px;">
                </h1>
                ';
    }
